...
; source: /Users/freeman/project/douyin/unidbg/unidbg-android/target/managed_program_dumps_350101_cf64_secondhop_bodies_20260904/350101_F43_0x61b400_0x840.bin
...
0008  +0x000c0  op=34  04 00 10 01  OR64             s16 = s4 | s0
...
000b  +0x00108  op=84  04 01 01 14  ADD64            s1 = s4 + s1
...
0015  +0x001f8  op=09  01 03 01 00  SUB32            s1 = sign_extend_32((uint32_t)s1 - (uint32_t)s3)
...
0019  +0x00258  op=34  10 00 04 00  OR64             s4 = s16 | s0
001a  +0x00270  op=5e  62 00 00 00  CALL_CF_INDEX    call native_binding[index=0x62] via q1 table ; q1=0x125fd360 rt/so-mapped
...
0020  +0x00300  op=84  10 01 04 04  ADD64            s4 = s16 + s1
0021  +0x00318  op=b5  00 01 20 00  ADD32_IMM16      s1 = int32(s0 +0x20)
0022  +0x00330  op=09  01 02 01 14  SUB32            s1 = sign_extend_32((uint32_t)s1 - (uint32_t)s2)
...
0028  +0x003c0  op=18  00 02 02 03  SHL32_IMM        s2 = (int32_t)(s2 << 3)
...
002b  +0x00408  op=84  01 02 02 00  ADD64            s2 = s1 + s2
002c  +0x00420  op=13  02 01 01 00  CMP_LO64         s1 = ((uint64_t)s2 < (uint64_t)s1) ? 1 : 0
...
003c  +0x005a0  op=84  10 11 01 14  ADD64            s1 = s16 + s17
...
0047  +0x006a8  op=68  00 03 04 08  LSR64_IMM        s4 = (uint64_t)s3 >> 8
0048  +0x006c0  op=26  01 03 27 00  ST8              *(s1 +0x27) = (uint8_t)s3
0049  +0x006d8  op=26  01 04 26 00  ST8              *(s1 +0x26) = (uint8_t)s4
004a  +0x006f0  op=68  03 03 04 10  LSR64_IMM        s4 = (uint64_t)s3 >> 16
004b  +0x00708  op=68  03 03 03 18  LSR64_IMM        s3 = (uint64_t)s3 >> 24
...
0050  +0x00780  op=5e  62 00 00 00  CALL_CF_INDEX    call native_binding[index=0x62] via q1 table ; q1=0x125fd360 rt/so-mapped
0051  +0x00798  op=34  1e 00 1d 01  OR64             s29 = s30 | s0
