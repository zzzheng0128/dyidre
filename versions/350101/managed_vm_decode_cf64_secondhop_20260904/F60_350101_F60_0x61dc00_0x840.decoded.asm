; MetaSec managed bytecode decode: F60
...
000b  +0x00108  op=84  04 01 01 04  ADD64            s1 = s4 + s1
...
0013  +0x001c8  op=84  10 01 04 00  ADD64            s4 = s16 + s1
...
001a  +0x00270  op=5e  66 00 00 00  CALL_CF_INDEX    call native_binding[index=0x66] via q1 table ; q1=0x125fd360 rt/so-mapped
...
002b  +0x00408  op=84  01 02 02 04  ADD64            s2 = s1 + s2
...
004b  +0x00708  op=68  13 03 03 18  LSR64_IMM        s3 = (uint64_t)s3 >> 24
...
0050  +0x00780  op=5e  66 00 00 00  CALL_CF_INDEX    call native_binding[index=0x66] via q1 table ; q1=0x125fd360 rt/so-mapped
