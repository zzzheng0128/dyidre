; MetaSec managed bytecode decode: F78
...
0005  +0x00078  op=53  01 08 80 08  LD_POOL_PTR      s8 = *(uint64_t *)q1 + 0x880 ; q1=0x125fd3a8 rt/so-mapped
...
001d  +0x002b8  op=18  00 0e 01 01  SHL32_IMM        s1 = (int32_t)(s14 << 1)
...
0021  +0x00318  op=0d  03 0c 01 11  BYTE_FROM_U32_SHIFT s3 = (uint8_t)((uint32_t)s12 >> (s1 & 31))
0022  +0x00330  op=17  06 0c 0c 07  SHL32_VAR        s12 = (int32_t)((uint32_t)s12 << ((uint32_t)s6 & 31))
0023  +0x00348  op=84  0a 0b 0b 00  ADD64            s11 = s10 + s11
0024  +0x00360  op=85  02 02 01 00  ADD64_IMM16      s2 = s2 +0x1
0025  +0x00378  op=b3  0d 01 01 01  AND64            s1 = s13 & s1
0026  +0x00390  op=36  0e 00 0d 01  NOR64            s13 = ~(s14 | s0)
0027  +0x003a8  op=b3  0c 0d 0c 00  AND64            s12 = s12 & s13
0028  +0x003c0  op=34  0c 01 01 01  OR64             s1 = s12 | s1
