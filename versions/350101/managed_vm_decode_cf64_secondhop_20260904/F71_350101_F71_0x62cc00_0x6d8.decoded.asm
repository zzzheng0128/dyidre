; MetaSec managed bytecode decode: F71
...
0010  +0x00180  op=b4  01 02 02 02  ADD32            s2 = int32(s1 + s2)
...
0024  +0x00360  op=34  11 00 05 00  OR64             s5 = s17 | s0
0025  +0x00378  op=34  13 00 06 00  OR64             s6 = s19 | s0
0026  +0x00390  op=67  00 01 01 00  LSR64_IMM32PLUS  s1 = (uint64_t)s1 >> (0 + 32)
0027  +0x003a8  op=84  14 01 04 14  ADD64            s4 = s20 + s1
...
002a  +0x003f0  op=34  14 00 05 00  OR64             s5 = s20 | s0
002b  +0x00408  op=5e  69 00 00 00  CALL_CF_INDEX    call native_binding[index=0x69] via q1 table ; q1=0x125fd360 rt/so-mapped
...
0031  +0x00498  op=5e  69 00 00 00  CALL_CF_INDEX    call native_binding[index=0x69] via q1 table ; q1=0x125fd360 rt/so-mapped
