...
# ── 结构速查 ─────────────────────────────────────────────────────────────
# 数据布局：VTAB=伪造 JNIEnv vtable（每项指向 VWIN+idx*16）；FAKEOBJ=伪造
#   Java 对象；CANNED=内置 URL 字符串（给 GetStringUTFChars 返回真实内容）
# 函数速查：
#   sym_name()       dynsym 下标 → 符号名
#   do_malloc()      堆模拟（bump 分配）
#   rd_str()         模拟器内存读 C 字符串
#   ret_to_lr()      设置返回值并按 LR 返回（导入模拟的公共出口）
#   handle_import()  PLT 存根 → 按符号名模拟 libc/pthread/JNI 语义
#   classify()       按 ZONES 地址区间把 BLR 目标归类（sign/orch/registry…）
#   hook_code()      取指回调：统计覆盖、记录 BLR 解析目标、VWIN 取指=JNI 调用
#   hook_intr()      svc #0 → 返回 0
#   hook_invalid()   未映射读写 → 补零页
#   hook_fetch_invalid()  ★关键：取指落空 = 空调用；必须 emu_stop 交外层
#     恢复循环重定向（在钩子里直接改 PC 不可靠，这是 R5 踩坑后的结论）
#   _redirect_ret() / _redirect_import()  软陷阱化：abort/金丝雀/导入的受控返回
#   emu_run()        单轮模拟：context 快照恢复 → 跑到 stop 或计数耗尽
# 主流程：先跑全部 116 个 .init_array ctor（INIT_ARRAY=0x260190），
#   建 context 快照，再逐操作码从 0x597D4 重跑（快照复用避免重复初始化）。
# ────────────────────────────────────────────────────────────────────────
def sym_name(idx):
...
def hook_fetch_invalid(mu_, access, address, size, value, user):
    if HOOKWIN <= address < HOOKWIN + hook_count[0] * 16:
        ...
        _redirect_import(mu_, hook_idx2name.get(idx, "?"), address)
        return True
    if VWIN <= address < VWIN + 1024 * 16:
        ...
        _redirect_ret(mu_, CANNED if slot == 168 else FAKEOBJ)
        return True
...
resume_pc = [None]   # fetch 钩子 redirect 计划

def _redirect_ret(mu_, val):
    """fetch-invalid 里的安全重定向：emu_stop + 外层循环从 LR 重启。"""
    mu_.reg_write(UC_ARM64_REG_X0, val)
    resume_pc[0] = mu_.reg_read(UC_ARM64_REG_X30)
    mu_.emu_stop()

def _redirect_import(mu_, name, va):
    """导入函数：先按原语义算返回值（直接改 X0），再走重定向。"""
    # handle_import 内部会 ret_to_lr 设 PC——这里只要 X0 语义,
    # 借用它对寄存器/内存的副作用, 然后统一走 emu_stop 重定向
    handle_import(name, va)          # 内部 ret_to_lr 设了 PC=LR（无所谓, 会被覆盖）
    resume_pc[0] = mu_.reg_read(UC_ARM64_REG_X30)
    mu_.emu_stop()

def emu_run(start, stop, count):
    """emu_start 的恢复循环：fetch 钩子 emu_stop 后从 resume_pc 继续。"""
    budget = count
    pc = start
    while budget > 0:
        resume_pc[0] = None
        try:
            mu.emu_start(pc, stop, count=budget)
            return "returned"
        except UcError as e:
            if resume_pc[0] is not None:
                pc = resume_pc[0]
                budget -= 1000   # 近似扣减
                continue
            pcv = mu.reg_read(UC_ARM64_REG_PC)
            return f"stop {e} @ {pcv - BASE:#x}"
...
for i in range(INIT_COUNT):
    ...
    mu.reg_write(UC_ARM64_REG_SP, STACK + 0x100000)
    st = emu_run(ctor, STOP, 2000000)
    if st == "returned":
        ctor_ok += 1
    else:
        ctor_fail.append((i, st[:70]))
...
for op1, op2 in combos:
    ...
    mu.reg_write(UC_ARM64_REG_X30, STOP)
    status = emu_run(BASE + JNI_BODY, STOP, 3000000)
    ...
    KEY = {"Inner_350": 0x149CA8, "orch_14F8C8": 0x14F8C8, "Callback_350": 0x14DBF4,
           "disp_14EBB4": 0x14EBB4, "disp_14E888": 0x14E888, "sub_8D6D8": 0x8D6D8,
           "sub_8EFD8": 0x8EFD8, "signv5_14F94C": 0x14F94C, "XPluto_150D38": 0x150D38,
           "hdr_419D8": 0x419D8}
    allpcs = set().union(*zone_cov.values()) if zone_cov else set()
    hit = [k for k, v in KEY.items() if any(v <= p < v + 0x900 for p in allpcs)]
    print("   KEY funcs covered:", hit)
    if nullcalls:
