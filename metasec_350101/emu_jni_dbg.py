...
# -*- coding: utf-8 -*-
# ═══════════════════════════════════════════════════════════════════════
# 【已废弃 · 仅留档】本文件是 emu_jni.py 开发期的第 1 个调试迭代（dbg）。
# 正式版本是 emu_jni.py（同目录），请直接使用它；本文件不要复用、不要维护。
# 保留原因：排查「fetch-invalid 钩子里改 PC 不可靠，必须 emu_stop + 外层
# 恢复循环」这一问题时的中间现场，排障时可对照。详见 ida_campaign_round5_report.md。
# ═══════════════════════════════════════════════════════════════════════
"""
...
def _hook_code_impl(mu_, addr, size, user):
    va = addr - BASE
    exec_count[0] += 1
...
def _fetch_impl(mu_, access, address, size, value, user):
...
def hook_code(mu_, addr, size, user):
    try:
        _hook_code_impl(mu_, addr, size, user)
    except Exception:
...
def hook_fetch_invalid(mu_, access, address, size, value, user):
    try:
        return _fetch_impl(mu_, access, address, size, value, user)
    except Exception:
        import traceback; traceback.print_exc()
        raise
...
for op1, op2 in combos:
    ...
    state_trace.clear(); zone_cov.clear(); nullcalls.clear()
    exec_count[0] = 0
    mu.reg_write(UC_ARM64_REG_SP, STACK + 0x100000)
