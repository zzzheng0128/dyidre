...
这个文件由 `scripts/index_true_env_runs.py` 生成。
...
| `true_env_xmedusa_summary.json` | `scripts/extract_true_env_xmedusa.py` 从原始 log 归一化 | 机器可读摘要，喂给 unidbg baseline 和差异分析。 |
...
| `f8_*` / `xmedusa_*` | `scripts/extract_true_env_xmedusa.py` | F8 入参、token、stub、lastF8、emit bytes/b64，专门追 `X-Medusa` 值级差异。 |
