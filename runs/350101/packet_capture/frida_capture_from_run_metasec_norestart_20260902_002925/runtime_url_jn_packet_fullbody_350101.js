globalThis.METASEC_PACKET_CONFIG = {
  "taskName": "frida_capture_from_run_metasec_norestart_20260902_002925",
  "filterAll": true,
  "captureBodies": true,
  "captureResponseBody": false,
  "excludeStaticTraffic": true,
  ...
  "hookInputStreamRead": false,
  "emitOnReadAfterBody": false,
  "emitOnGetInputStreamAfterBody": true,
  "emitOnBodyIdle": false,
  "useJavaIdleTimer": false,
  "emitEmptyPostDisconnect": true,
  "emitBodylessPostDisconnect": true,
...
(function () {
  ...
  var SSCRONET_JSTRING_TO_STD_STRING_THUNK_OFF = 0x31edc8; // x8 -> x2, then tail-branches.
  var SSCRONET_JSTRING_TO_STD_STRING_OFF = 0x31ebf4;       // real ConvertJavaStringToUTF8, out std::string is x2.
  ...
  var JNI_GET_STRING_LENGTH = 164;
  var JNI_GET_STRING_CHARS = 165;
  var JNI_RELEASE_STRING_CHARS = 166;
  ...
  var options = {
    ...
    excludeStaticTraffic: cfg.excludeStaticTraffic === undefined ? true : !!cfg.excludeStaticTraffic,
    ...
    hookInputStreamRead: cfg.hookInputStreamRead === undefined ? false : !!cfg.hookInputStreamRead,
    copyResponseHeaders: cfg.copyResponseHeaders === undefined ? true : !!cfg.copyResponseHeaders,
    hookNativeJN: cfg.hookNativeJN === undefined ? false : !!cfg.hookNativeJN,
    hookNativeJNConverter: cfg.hookNativeJNConverter === undefined ? false : !!cfg.hookNativeJNConverter,
    ...
    emitOnReadAfterBody: cfg.emitOnReadAfterBody === undefined ? false : !!cfg.emitOnReadAfterBody,
    ...
    nativeStringDumpLimit: Number(cfg.nativeStringDumpLimit || 0),
  ...
  var state = globalThis.__rfPacket350101 || {
    ...
    stats: {
      ...
      nativeJNDirectStringReads: 0,
      nativeJNStringDumps: 0,
      mergedByUrl: 0,
      jnMethodCalls: 0,
      jnHeaderCalls: 0,
      writeCalls: 0,
  ...
  function readCStringSafe(p, maxLen) {
    try {
      var q = ptr(p);
      if (q.isNull()) return "";
      if (typeof q.readCString === "function") return q.readCString(maxLen || 65536) || "";
      if (typeof Memory !== "undefined" && Memory && typeof Memory.readCString === "function") {
        return Memory.readCString(q, maxLen || 65536) || "";
      }
      return "";
    } catch (_) {
      return "";
  ...
  function memReadU8(p) {
    var q = ptr(p);
    if (typeof q.readU8 === "function") return q.readU8();
    if (typeof Memory !== "undefined" && Memory && typeof Memory.readU8 === "function") return Memory.readU8(q);
    var ab = null;
    if (typeof q.readByteArray === "function") ab = q.readByteArray(1);
    else if (typeof Memory !== "undefined" && Memory && typeof Memory.readByteArray === "function") ab = Memory.readByteArray(q, 1);
    if (ab) return (new Uint8Array(ab))[0];
    throw new Error("no byte reader");
  }

  function memReadPointer(p) {
    var q = ptr(p);
    if (typeof q.readPointer === "function") return q.readPointer();
    if (typeof Memory !== "undefined" && Memory && typeof Memory.readPointer === "function") return Memory.readPointer(q);
    throw new Error("no pointer reader");
  }

  function memReadByteArray(p, len) {
    var q = ptr(p);
    if (typeof q.readByteArray === "function") return q.readByteArray(Number(len || 0));
    if (typeof Memory !== "undefined" && Memory && typeof Memory.readByteArray === "function") {
      return Memory.readByteArray(q, Number(len || 0));
    }
    return null;
  }
  ...
  function readStdString(p, maxLen) {
    try {
      ...
      var marker = memReadU8(s.add(0x17));
      var data = (marker & 0x80) ? memReadPointer(s) : s;
  ...
  function readBytesHex(p, len) {
    try {
      var q = ptr(p);
      if (q.isNull()) return "";
      var ab = memReadByteArray(q, Number(len || 32));
      if (!ab) return "";
      var u = new Uint8Array(ab);
      var out = [];
      for (var i = 0; i < u.length; i++) out.push(hex2(u[i]));
      return out.join("");
    } catch (e) {
      return "err:" + e;
    }
  ...
  function stdStringDebug(p) {
    try {
      var s = ptr(p);
      if (s.isNull()) return "out=0";
      var marker = 0;
      try { marker = memReadU8(s.add(0x17)); } catch (_) {}
      var p0 = "0";
      var p8 = "0";
      var p16 = "0";
      try { p0 = pointerText(memReadPointer(s)); } catch (_) {}
      try { p8 = pointerText(memReadPointer(s.add(8))); } catch (_) {}
      try { p16 = pointerText(memReadPointer(s.add(16))); } catch (_) {}
      var heap = "";
      try { heap = readCStringSafe(memReadPointer(s), 96); } catch (_) {}
      return "out=" + s +
        " marker=0x" + hex2(marker) +
        " p0=" + p0 + " p8=" + p8 + " p16=" + p16 +
        " inline_hex=" + readBytesHex(s, 32) +
        " inline_cstr=" + shortText(readCStringSafe(s, 96), 96) +
        " heap_cstr=" + shortText(heap, 96);
    } catch (e) {
      return "stdStringDebug failed: " + e;
    }
  }
  ...
  function configureStealth() {
    try {
      if (typeof Java.setStealth !== "function") {
        log("java stealth unavailable, running stock Frida Java API");
        return;
      }
      var before = -1;
  ...
  function isRustFridaJavaHookApi() {
    try {
      return typeof Java !== "undefined" && Java &&
        typeof Java.setStealth === "function" &&
        typeof Hook !== "undefined" && Hook && Hook.WXSHADOW !== undefined;
    } catch (_) {
      return false;
    }
  }

  function setJavaImpl(overload, fn) {
    if (!overload) throw new Error("missing Java overload");
    if (isRustFridaJavaHookApi()) {
      overload.impl = fn;
      return;
    }
    if (typeof overload.call !== "function") {
      throw new Error("stock Frida overload.call unavailable");
    }
    overload.implementation = function () {
      var self = this;
      var hadOrig = false;
      var oldOrig = undefined;
      try {
        hadOrig = Object.prototype.hasOwnProperty.call(self, "$orig");
        oldOrig = self.$orig;
      } catch (_) {}
      try {
        self.$orig = function () {
          ...
          return overload.call.apply(overload, argv);
        };
      } catch (eSetOrig) {
        addEvent("set $orig shim failed: " + eSetOrig);
      }
      try {
        return fn.apply(self, arguments);
      } finally {
        try {
          if (hadOrig) self.$orig = oldOrig;
          else delete self.$orig;
        } catch (_) {}
      }
    };
  }
  ...
  function isStaticOrMediaTraffic(url) {
    var s = safeString(url).toLowerCase();
    if (!s) return false;
    if (s.indexOf("douyinpic.com") >= 0) return true;
    if (s.indexOf("douyinvod.com") >= 0) return true;
    if (s.indexOf("huoshanimg.com") >= 0) return true;
    if (s.indexOf("byteimg.com") >= 0) return true;
    if (s.indexOf("byteeffecttos.com") >= 0) return true;
    if (s.indexOf("bytedanceeffect.com") >= 0) return true;
    if (s.indexOf("effectcdn") >= 0) return true;
    if (s.indexOf("sourcecdn") >= 0) return true;
    if (s.indexOf("dig.bdurl.net/") >= 0) return true;
    if (s.indexOf("effect.snssdk.com/model/") >= 0) return true;
    if (s.indexOf("bytegecko.com/obj/") >= 0) return true;
    if (s.match(/\.(heic|png|jpg|jpeg|webp|gif|mp4|m4a|mp3|aac|zip|ttf|otf|woff|woff2)(\?|$)/)) return true;
    return false;
  }
  ...
  function isTargetUrl(url) {
    ...
    if (options.excludeStaticTraffic && isStaticOrMediaTraffic(s)) return false;
  ...
  function installJNHooks() {
    ...
    try {
      ...
      setJavaImpl(createReq, function (a0, a1, url, a3, a4, a5, a6, a7, a8, a9, a10, a11, a12) {
      ...
      setJavaImpl(setMethod, function (urlPtr, obj, method) {
      ...
      setJavaImpl(addReqHeader, function (urlPtr, obj, key, value) {
        var req = matchUrlPtr(urlPtr);
        if (req) {
    ...
    try {
      ...
      Interceptor.attach(convAddr, {
        ...
        onLeave: function (retval) {
          ...
          if (!text && this.jniText) text = this.jniText;
          if (!text && state.stats.nativeJNStringDumps < options.nativeStringDumpLimit) {
            state.stats.nativeJNStringDumps++;
            log("sscronet stdstr dump site=" + this.site +
              " jni_len=" + safeString(this.jniText).length + " " + stdStringDebug(this.out));
          }
      ...
      log("hooked " + SSCRONET_MODULE +
        ...
        " thunk=0x" + SSCRONET_JSTRING_TO_STD_STRING_THUNK_OFF.toString(16) +
  ...
  function hookSSCronetNativeJNAtBase(base) {
    ...
    try {
      ...
      if (options.hookNativeJNConverter) hookSSCronetStringConverterAtBase(basePtr);
      else log("skip " + SSCRONET_MODULE + " jstring->std::string converter hook hookNativeJNConverter=false");
  ...
  function installCronetHooks() {
    try {
      ...
      setJavaImpl(write, function (bytes, off, len) {
        var req = null;
        var url = "";
        try {
    ...
    if (options.hookInputStreamRead || options.captureResponseBody || options.emitOnReadAfterBody) {
      try {
        var In = Java.use("com.ttnet.org.chromium.net.urlconnection.CronetInputStream");
        var read = In.read.overload("[B", "int", "int");
        setJavaImpl(read, function (bytes, off, len) {
      ...
      log("skip CronetInputStream.read hook hookInputStreamRead=false captureResponseBody=false emitOnReadAfterBody=false");
    }
    ...
    try {
      ...
      if (options.hookConnections) {
        try {
          ...
          setJavaImpl(setRequestMethod, function (method) {
        ...
        try {
          ...
          setJavaImpl(addRequestProperty, function (key, value) {
        ...
        try {
          ...
          setJavaImpl(setRequestProperty, function (key, value) {
            var ret = this.$orig(key, value);
            try {
              var url = urlFromConnection(this);
        ...
        try {
          ...
          setJavaImpl(getOutputStream, function () {
            var url = "";
            var req = null;
            try {
        ...
        try {
          ...
          setJavaImpl(getInputStream, function () {
            try {
              var url = urlFromConnection(this);
      ...
      setJavaImpl(disconnect, function () {
        var req = null;
        try {
          var url = urlFromConnection(this);
  ...
  old.urljnsummary = function () {
    return {
      ...
      hookNativeJNConverter: options.hookNativeJNConverter,
  ...
  try {
    installWhenJavaReady();
  } catch (e) {
    state.stats.errors++;
    addEvent("Java install schedule failed: " + e);
    log("Java install schedule failed, installing directly: " + e);
    install();
  }
})();
