globalThis.METASEC_PACKET_CONFIG = {
  "taskName": "nativejn_sscronet_20260901_233207",
  "filterAll": true,
  "captureBodies": true,
...
(function () {
    ...
    try {
      ...
      Interceptor.attach(createAddr, {
        onEnter: function (args) {
          this.url = readJStringSafe(args[0], args[4], "sscronet.create.url");
        },
        onLeave: function (retval) {
          var ptrText = pointerText(retval);
          var urlText = safeString(this.url);
          state.stats.create++;
      ...
      Interceptor.attach(methodAddr, {
        onEnter: function (args) {
          ...
          var method = readJStringSafe(args[0], args[4], "sscronet.method");
          var req = matchUrlPtr(reqPtr);
          if (req) {
          ...
          diagLog("sscronet", state.stats.nativeJNMethodCalls, options.logJNEvery,
            "setMethod ptr=" + reqPtr + " matched=" + (req ? 1 : 0) +
            " method=" + method);
        }
      }, desiredStealthValue());
      ...
      Interceptor.attach(headerAddr, {
        onEnter: function (args) {
          ...
          var reqPtr = pointerText(args[2]);
          var key = readJStringSafe(args[0], args[4], "sscronet.header.key");
          var value = readJStringSafe(args[0], args[5], "sscronet.header.value");
          var req = matchUrlPtr(reqPtr);
          if (req) {
            addHeader(req, key, value);
            state.stats.header++;
          ...
          diagLog("sscronet", state.stats.nativeJNHeaderCalls, options.logJNEvery,
            "addHeader ptr=" + reqPtr + " matched=" + (req ? 1 : 0) +
            " key=" + key + " value_len=" + value.length);
        }
      }, desiredStealthValue());
  ...
  function installSSCronetNativeJNHooks() {
    ...
    var base = Module.findBaseAddress(SSCRONET_MODULE);
