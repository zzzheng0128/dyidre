globalThis.METASEC_PROBE_CONFIG = {"mode": "packet-capture", "packet": {"uploadUrl": "http://127.0.0.1:8891/up/dy/packets", "version": "35.1.0", "taskName": "e2e_packet_normal", "javaStealth": "normal", "requireJavaStealth": false, "maxBodyBytes": 4194304, "filterAll": true}};
...
(function () {
    ...
    //     console.log("cronet base=" + cronet)
    ...
    //         if (funarr.indexOf(arg3.toString()) == -1) {
    //             funarr.push(arg3.toString())
    //             hookCallBack(arg3);
    //             // console.log('hook custom_verify ' + arg3 + ' done!');
    //         }
...
// ===== mode: packet-capture / Cronet packet 组包上传 =====
// 对齐 xpmodules/ldouyin 的 SSCronet313Hook + main.cpp 0x14DBF4：
//   - J.N.MnXVOzVo/MfdvbiJC/MtJFji5x 记录 URL/method/request headers；
//   - CronetFixedModeOutputStream.write / CronetInputStream.read 记录 body；
//   - libmetasec_ml.so +0x14DBF4 合并 MetaSec X-* headers；
//   - disconnect 时上传到 dydcd /up/dy/packets。
...
(function () {
    ...
    state.options = {
        ...
        filters: opt.filters || DEFAULT_FILTERS,
        ...
        maxPending: Number(opt.maxPending || 256),
        maxBodyBytes: Number(opt.maxBodyBytes === undefined ? 4 * 1024 * 1024 : opt.maxBodyBytes),
    ...
    function ensureRequestForUrl(url, reason) {
        ...
        addEvent("auto track " + safeString(reason) + " url=" + s);
        return req;
    }
    ...
    function addXmHeaders(nativeUrl, xms) {
        ...
        if (!req) {
            addEvent("xm no match url=" + safeString(nativeUrl));
            return;
        }
        ...
        log("xm matched url=" + req.url + " keys=" + Object.keys(headers).join(","));
    }
    ...
    function ensureUploaderClass() {
        ...
        state.uploaderClass = Java.registerClass({
            name: "com.dyidre.frida.PacketUploadRunnable" + VERSION_CODE + "_" + Process.id + "_" + Date.now(),
            implements: [Runnable],
            fields: {
            ...
            methods: {
                $init: [{
                    ...
                    implementation: function (targetUrl, body) {
                ...
                run: function () {
                    try {
                        var code = postJson(safeString(this.targetUrl.value), safeString(this.body.value));
    ...
    function uploadRequest(req) {
        ...
        if (!state.options.upload) return;
        var body = JSON.stringify({ device_id: state.deviceId || "", packets: [one] });
        try {
            var Thread = Java.use("java.lang.Thread");
            var Uploader = ensureUploaderClass();
            var runnable = Uploader.$new(state.options.uploadUrl, body);
            var thread = Thread.$new(runnable, "dyidre-packet-upload");
            thread.start();
            state.stats.uploadStarted++;
            log("upload start bytes req=" + Math.floor((req.requestBodyHex || "").length / 2) +
                " resp=" + Math.floor((req.responseBodyHex || "").length / 2) +
                " url=" + req.url);
        } catch (e) {
            state.stats.uploadFailed++;
            log("upload start failed: " + e);
        }
    }
        ...
        try {
            ...
            disconnect.impl = function () {
                try {
                    ...
                    var req = matchUrl(url, currentThreadInfo()) || ensureRequestForUrl(url, "disconnect");
                    if (req) {
                    ...
                    addEvent("disconnect capture failed: " + e);
                }
                return this.$orig();
            };
    ...
    function installNativeAtBase(base) {
        ...
        try {
            ...
            }, (typeof Hook !== "undefined" && Hook.NORMAL !== undefined) ? Hook.NORMAL : 0);
...
(function () {
    ...
    var MODES = {
        ...
        "packet-capture": { run: __dyidre_mode_packet_capture, source: "metasec_probe_350101.js#packet-capture", desc: "Cronet 请求/响应组包 + 0x14DBF4 X-* header + dydcd 上传" },
    ...
    var ALIASES = {
        ...
        "cronet": "packet-capture",
        "dydcd": "packet-capture",
        "maps": "artcheck",
        "art": "artcheck",
