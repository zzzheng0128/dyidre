globalThis.METASEC_PACKET_CONFIG = {
  "taskName": "fullbody_nojn_native_20260901_224208",
  "filterAll": true,
  "captureBodies": true,
      ...
      if (options.hookConnections) {
        ...
        try {
          ...
          addRequestProperty.impl = function (key, value) {
            ...
            try {
              var req = ensureRequestForUrl(urlFromConnection(this), "addRequestProperty");
              if (req) {
                addHeader(req, key, value);
        ...
        try {
          ...
          setRequestProperty.impl = function (key, value) {
            var ret = this.$orig(key, value);
            try {
              var req = ensureRequestForUrl(urlFromConnection(this), "setRequestProperty");
              if (req) {
                addHeader(req, key, value);
                state.stats.connectionHeader++;
              }
        ...
        try {
          ...
          getOutputStream.impl = function () {
            var ret = this.$orig();
            try {
              var req = ensureRequestForUrl(urlFromConnection(this), "getOutputStream");
              if (req) {
