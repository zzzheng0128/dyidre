globalThis.METASEC_PROBE_CONFIG = {"mode": "ssl", "ssl": {"stealth": true, "requireStealth": true}};
    ...
    function hookCronetStreams() {
        if (state.options.hookStreams) try {
            ...
            write.impl = function (bytes, off, len) {
                ...
                var bodyHex = "";
                try {
                    if (state.options.captureBodies && !(state.options.skipMainThread && isMainThread())) {
                        var conn = getFieldValue(this, "mConnection");
                        var url = urlFromConnection(conn);
                        req = matchUrl(url, currentThreadInfo()) || ensureRequestForUrl(url, "stream.write");
                        if (req) bodyHex = javaByteArrayToHex(bytes, off, len);
                    } else if (state.options.captureBodies && state.options.skipMainThread && isMainThread()) {
                        state.stats.hookSkippedMain++;
                    }
                } catch (e) {
                    addEvent("write pre-capture failed: " + e);
                }
                var ret = this.$orig(bytes, off, len);
                try {
                    if (req && bodyHex) appendBodyHex(req, "requestBodyHex", bodyHex);
                } catch (e2) {
                    addEvent("write body append failed: " + e2);
                }
                return ret;
            };
...
(function () {
    ...
    function installNativeHook() {
        var base = Module.findBaseAddress(TARGET_MODULE);
        log("install native-only stealth=" + state.options.nativeStealth + " watchLoader=" + state.options.watchLoader + " base=" + base);
        if (base !== null) {
            installNativeAtBase(base);
            if (!state.options.watchLoader) return;
        }
