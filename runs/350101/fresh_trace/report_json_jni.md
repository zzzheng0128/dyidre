...
- 运行目录：`/Users/freeman/project/douyin/dyidre/runs/350101/fresh_trace`
...
| 事件头 | ascii |
|---|---|
| [W] addr=0x1236f580 PC=RX@0x1217c2bc[libc.so]0x1c2bc LR=RX@0x1248b4ec[libmetasec_ml.so]0x10b4ec run=22@31 | `d",.."app_version":."35.1.0",.."update_version_code":."35.1.0",.` |
| [W] addr=0x1236f590 PC=RX@0x1217c2c0[libc.so]0x1c2c0 LR=RX@0x1248b4ec[libmetasec_ml.so]0x10b4ec run=22@15 | `n":."35.1.0",.."update_version_code":."35.1.0",.."package_name":` |
| [W] addr=0x1236f5a0 PC=RX@0x1217c2c4[libc.so]0x1c2c4 LR=RX@0x1248b4ec[libmetasec_ml.so]0x10b4ec run=21@0 | `update_version_code":."35.1.0",.."package_name":."com.ss.android` |
...
## 五、IDA MCP 对照（port 13339, materials/350101/libmetasec_ml.so.i64）

trace 锚点 ↔ IDA 符号已双向确认，注释已写回 IDA（append_comments）：

| trace LR | 指令 | IDA 符号 | 语义（已反编译确认） |
|---|---|---|---|
| `0x14a520` | `0x14a51c: bl 0x14d44c` | `copyStringToMemBlock_saveX24_350` | 薄 wrapper → `copyStringMemBlock2`；拷贝 `[SP,#ptr]` 为 **key** MEM_BLOCK → X24 |
| — | `0x14a520: bl 0x14d264` | `newMemBlock24_350` | `newMem(0x18)` 分配 24 字节 MEM_BLOCK |
| `0x14a530` | `0x14a52c: bl 0x10b5f0` | `copyStringMemBlock2` | `{vtable=off_262AF0; mem_len=strlen+1; src_len=strlen; mem=malloc+memcpy}`；拷贝 `[SP,#s]` 为 **value** MEM_BLOCK → X25，trace 在其 memcpy 里抓到 proto wire 明文 |
| — | `0x14a53c: bl 0x11fc30` | `treeMapPut_350` | put(map=X22, key=X24, value=X25)，IDA 已有注释 `[emit] Direct treeMapPut for X-Medusa` |

- 宿主函数：`buildSignedHttpHeadersInner_350`（`0x149ca8`，size 0xad4）
- 其上一条（`0x14a4e8-0x14a508`）：`setUpdateSettingItemValue_int(map, "2e8ab1223d07836ad4fc65fc581b4808", 100)` —— setting 项，IDA 已有 [state-flow] 注释
- MEM_BLOCK 布局（24B）：`+0 vtable`、`+8 mem_len/src_len`、`+16 mem*`
