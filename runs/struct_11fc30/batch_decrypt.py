    ...
    for j in range(i - 1, max(i - 80, -1), -1):
        ...
        if m2.group(2) in ("bl", "b"):
            ...
            if t.startswith("0x1c2f40") or t.startswith("0x1c2edc"):
                ...
                boundary = boundary if boundary is not None else -j  # negative = "other call" marker
            break
    # ---- forward mini-interpret from boundary to site ----
    regs = {}      # w/x reg -> int
    fregs = {}     # d/q reg -> bytes
    retreg = "x0"  # register holding new[] result
    stores = []    # (base, off, size, int|bytes)
    length = None
    x0_src = None
    ...
    for j in range(start, i):
        ...
        if op in ("mov", "movk", "movz"):
            ...
            if im:
                ...
                continue
            mm = re.match(r"^(x\d+),\s*(x0)$", args)  # mov xR, x0 -> track new[] result
            if mm:
                retreg = mm.group(1)
                continue
            continue
...
for s in sites:
    ...
    try:
        ...
        pt = rd_str(res) if res else rd_str(buf)
